...
function FilteredCategories() {
	...
	useEffect(() => {
		if (selectedCategory) {
			fetch(`https://642ee23f2b883abc64198889.mockapi.io/purchases?category=${selectedCategory}`)
									...
									<PieChart width={300} height={300}>
										...
										>
											{items.map((item, index) => {
												const selectedMonth = new Date(item.date).getMonth() + 1; // +1, так как номер месяца начинается с 0
												const categoryColor = category.color[selectedMonth.toString()];
								...
								<div className="mt-4 w-1/3 mx-auto bg-white rounded-md font-['Source Sans Pro']">
									<div className="flex justify-between border-b-2 py-3 px-4 font-semibold">
										...
										<div className='text-red-600 text-xl font-bold'>
											{items.reduce((total, item) => total + item.price, 0)} руб.
										</div>
									</div>
								</div>
							</div>
